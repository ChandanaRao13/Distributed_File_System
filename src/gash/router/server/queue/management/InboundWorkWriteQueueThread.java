			...
			try {
				...
				if (internalNode != null) {
					...
					if (workMessage.getWorktype() == Worktype.REPLICATE_REQUEST) {
						if (DatabaseHandler.addFile(ft.getFilename(), ft.getChunkCounts(), ft.getChunk().toByteArray(),
								ft.getChunkNo())) {
						...
						if (DatabaseHandler.deleteFile(ft.getFilename())) {
							...
							QueueManager.getInstance().enqueueOutboundWorkWrite(workResponseMessage,
						...
						if (!DataReplicationManager.fileUpdateTracker.containsKey(filename)) {
							...
							if (DatabaseHandler.addFile(fileTask.getFilename(), fileTask.getChunkCounts(),
