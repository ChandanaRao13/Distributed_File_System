...
public class OutboundReadWorkMsgHandler extends Thread {
	private QueueManager manager;
	protected static Logger logger = LoggerFactory.getLogger(OutboundReadWorkMsgHandler.class);

	public OutboundReadWorkMsgHandler(QueueManager manager) {
		super();
		this.manager = manager;
		if (manager.outboundReadWorkQueue == null)
				...
				if (message != null) {
					if (message.getChannel() != null && message.getChannel().isOpen()) {
						if (message.getChannel().isWritable()) {
							...
							if (!cf.isSuccess()) {
								logger.info("Unsuccessful: failed to send chunk: "
										+ message.getWorkMessage().getFiletask().getChunkNo());
								manager.enqueueOutboundRead(message.getWorkMessage(), message.getChannel());
								logger.info("Adding back to read outbound queue");
							} else {
								// logger.info("cf.isSuccess() == true: " +
								// cf.isSuccess());
							}
						} else {
						...
						// logger.info("Adding back to read outbound queue: not
						// of message.getChannel()!= null &&
						// message.getChannel().isOpen()");
						// manager.returnOutboundWorkWriteQueue(message);
						manager.enqueueOutboundRead(message.getWorkMessage(), message.getChannel());
					}
				}
